...
} from '../../tests/helpers/filesystem.js'
...
describe('File System Class', async () => {
  ...
  beforeEach(async () => {
    ...
    await cabinet.addTicket(
      'file_system',
      await authority.clerk.tickets.fileSystem.create(Path.root(), did)
    )
  })
  ...
  it('writes and reads private files', async () => {
    ...
    const { capsuleKey } = await fs.write(path, 'json', { foo: 'bar', a: 1 })
  ...
  it('can get a capsule key for an existing private file', async () => {
    ...
    assert.equal(key ? JSON.stringify(key) : null, JSON.stringify(capsuleKey))
  })
  ...
  it('can get a capsule CID for a mounted private directory', async () => {
    ...
    assert.notEqual(key ? JSON.stringify(key) : null, null)
  })
