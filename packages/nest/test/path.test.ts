...
describe('Path functions', () => {
  ...
  it('creates directory paths', () => {
    ...
    // Type testing
    const a: Path.Directory<Path.Partitioned<Path.Private>> =
      Path.directory('private')
    const b: Path.Directory<Path.PartitionedNonEmpty<Path.Public>> =
  ...
  it('creates file paths', () => {
    ...
    // Type testing
    const a: Path.File<Path.PartitionedNonEmpty<Path.Private>> = Path.file(
      'private',
      'a'
    )
  ...
  it('creates directory paths with fromKind', () => {
    ...
    // Type testing
    const a: Path.Directory<Path.Partitioned<Path.Private>> = Path.fromKind(
      Path.Kind.Directory,
      'private'
    )
    const b: Path.Directory<Path.PartitionedNonEmpty<Path.Public>> =
      Path.fromKind(Path.Kind.Directory, 'public', 'a')
    const c: Path.Directory<Path.Segments> = Path.fromKind(
      Path.Kind.Directory,
      'private',
      'a',
      'b'
  ...
  it('creates file paths with fromKind', () => {
    ...
    // Type testing
    const a: Path.File<Path.PartitionedNonEmpty<Path.Private>> = Path.fromKind(
      Path.Kind.File,
      'private',
      'a'
    )
    const b: Path.File<Path.Segments> = Path.fromKind(
      Path.Kind.File,
      'private',
      'a',
      'b'
  ...
  it('can be combined', () => {
    ...
    // Type testing
    const a: DirectoryPath<Path.PartitionedNonEmpty<Path.Private>> =
    ...
    const c: DirectoryPath<Path.PartitionedNonEmpty<Path.Private>> =
      Path.combine(Path.directory('private'), Path.directory('a'))
  ...
  it('supports parent', () => {
    ...
    assert.equal(Path.parent(Path.root()), null)

    // Type testing
    const a: DirectoryPath<Path.PartitionedNonEmpty<Path.Partition>> =
      Path.parent({
  ...
  it('supports replaceTerminus', () => {
    ...
    const b: FilePath<Path.PartitionedNonEmpty<Path.Partition>> =
      Path.replaceTerminus(
        {
          file: ['private', 'a'],
        },
