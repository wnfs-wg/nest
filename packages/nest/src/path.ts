 ...
 * A directory path.
 */
export interface DirectoryPath<P> {
  directory: P
}
...
export function fromKind(
  ...
  return kind === Kind.Directory ? directory(...args) : file(...args)
}
...
export function appData<P extends Partition>(
  ...
  return suffix === undefined ? appDir : combine(appDir, suffix)
}
...
export function isDirectory<P>(
  path: DistinctivePath<P>
): path is DirectoryPath<P> {
 ...
 */
export function isOnRootBranch(
...
export function isPartitioned<P extends Partition>(
  ...
  return [RootBranch.Private, RootBranch.Public, 'private', 'public'].includes(
    soCalledPartition
  )
}
...
export function parent(
  path: DistinctivePath<[Segment, Segment, Segment, ...Segments]>
): DirectoryPath<SegmentsNonEmpty>
export function parent(
  path: DistinctivePath<PartitionedNonEmpty<Partition>>
): DirectoryPath<Partitioned<Partition>>
export function parent(
  path: DistinctivePath<[Partition, Segment]>
): DirectoryPath<Partitioned<Partition>>
export function parent(
...
export function replaceTerminus(
  ...
  return combine(parent(path), fromKind(kind(path), terminus))
}

export function rootBranch(
  path: DistinctivePath<Segments>
): { branch: RootBranch; rest: Segments } | undefined {
...
export function terminus(path: DistinctivePath<Segments>): string | undefined
export function terminus(path: DistinctivePath<Segments>): string | undefined {
  const u = unwrap(path)
