...
import * as RootTree from './root-tree.js'
...
export class FileSystem {
  ...
  static async empty(opts: FileSystemOptions<FileSystem>): Promise<FileSystem> {
    const { blockStore, did, settleTimeBeforePublish } = opts
  ...
  static async fromCID(
    cid: CID,
    opts: FileSystemOptions<FileSystem>
  ): Promise<FileSystem> {
    const { blockStore, did, settleTimeBeforePublish } = opts

    const rootTree = await RootTree.fromCID({
      blockStore,
      cid,
    })
   ...
   * @group Events
   */
  on = <Name extends keyof Events.FileSystem>(
    eventName: Name,
    listener: EventListener<Events.FileSystem, Name>
   ...
   * @group Events
   */
  off = <Name extends keyof Events.FileSystem>(
    eventName: Name,
    listener: EventListener<Events.FileSystem, Name>
   ...
   * @group Events
   */
  once = <Name extends keyof Events.FileSystem>(eventName: Name) =>
   ...
   * @group Events
   */
  events = <Name extends keyof Events.FileSystem>(eventName: Name) =>
    this.#eventEmitter.events(eventName)
  ...
  async transaction(
    ...
    // Commit transaction
    const { changes, privateNodes, proofs, rootTree } =
    ...
    // Publish
    const signal =
      mutationOptions.skipPublish === true
      ...
      case 'public': {
        const node =
          partition.segments.length === 0
            ...
            : await this.#rootTree.publicRoot.getNode(
                partition.segments,
                this.#blockStore
              )
        if (!node)
  ...
  static readonly #statusNotPublishing: PublishingStatus = {
    persisted: false,
  ...
  readonly #debouncedDataRootUpdate = debounce(
    async (
      args: Array<[dataRoot: CID, proofs: Ticket[]]>
      ...
      const rootUpdate = await this.#updateDataRoot(dataRoot, proofs)
   ...
   */
  async #publish(dataRoot: CID, proofs: Ticket[]): Promise<PublishingStatus> {
