...
export function findPrivateNode(
  ...
  for (let i = 0; i <= pathSegments.length; i++) {
    ...
    const result: MountedPrivateNode | undefined =
      privateNodes[Path.toPosix(path, { absolute: true })]
...
export function partition(path: Path.Distinctive<Partitioned<Partition>>): {
  name: 'public' | 'private'
