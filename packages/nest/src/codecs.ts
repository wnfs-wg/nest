...
export function getByCode(code: number): BlockCodec<number, any> {
  ...
  if (codec === undefined) {
    throw new Error(
      `No codec was registered for the code: ${numberHex(
        code
...
export function expect(codecId: CodecIdentifier, cid: CID): void {
  ...
  if (cid.code !== codec.code) {
    ...
    throw new Error(
      `Expected a ${codec.name} CID, found a ${cidCodec.name} CID instead.`
    )
  }
}
...
): codeOrName is CodecIdentifier {
  return typeof codeOrName === 'string'
    ? BY_NAME[codeOrName] !== undefined
    : BY_CODE[codeOrName] !== undefined
}
